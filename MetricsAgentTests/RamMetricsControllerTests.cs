...
public class RamMetricsControllerTests
{
    ...
    public RamMetricsControllerTests()
    {
        ...
        _controller = new RamMetricsController(loggerMock.Object, _repositoryMock.Object, mapperMock.Object);
    ...
    public void Create_ShouldCall_Create_From_Repository()
    {
        ...
        var result = _controller.Create(new MetricsAgent.Requests.RamMetricCreateRequest()
            {Time = TimeSpan.FromSeconds(1), Value = 50});
        _repositoryMock.Verify(repository =>repository.Create(It.IsAny<RamMetric>()), Times.AtLeastOnce());
    }
    
    [Fact]
    public void Update_ShouldCall_Update_From_Repository()
    {
        var result = _controller.Update(new RamMetric(){ Id = 1, Time = TimeSpan.FromSeconds(1), Value = 20});
        Assert.IsAssignableFrom<IActionResult>(result);
    }
